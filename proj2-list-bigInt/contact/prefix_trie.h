...
struct node{ struct node* s[256]; int leaf; vector vec; };
...
vector Find(Prefix_Trie t,const char* str);
...
vector Find_(Prefix_Trie_iterator t,const char* str);
...
void Insert_(Prefix_Trie_iterator *t,const char* str,const char* mn)
{
	...
	if(str[0]==0)
	{
		char* p=(char*)malloc(sizeof(char)*(strlen(mn)+1));
		strcpy(p,mn);
		if(!(*t)->vec)(*t)->vec=New_vector();
		(*t)->leaf=1;
		Push_back((*t)->vec,p);
		return ;
	}
	Insert_(&((*t)->s[(unsigned char)str[0]]),str+1,mn);
}
...
vector Find_(Prefix_Trie_iterator t,const char* str)
{
	...
	if(str[0]==0)
	{
		if(t->leaf) return t->vec;
		else return 0;
	}
	return Find_(t->s[(unsigned char)str[0]],str+1);
}
