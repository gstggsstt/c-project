...
int ilog2(int x) { x=x|(x>>1); x=x|(x>>2); x=x|(x>>4);
	x=x|(x>>8); x=x|(x>>16); return x+1; }

/*allocate a new vector*/
vector New_vector()
{
	vector t=(vector)calloc(1,sizeof(struct vector_));
	t->size=0,t->mx_size=1;
	t->data=(vec_base*)calloc(1,sizeof(vec_base));
...
{
	v->mx_size=ilog2(x+1);
	__tp_vector* t=(__tp_vector*)calloc(v->mx_size,sizeof(__tp_vector));
	/*Copy data*/
	memcpy(t,v->data,
		sizeof(__tp_vector)*v->size);
	v->size=x;
	free(v->data);
	v->data=t; v->begin=t;
...
{
	vector u=(vector)calloc(1,sizeof(struct vector_));
	u->mx_size=ilog2(v->size+1); u->size=v->size;
	u->data=(__tp_vector*)calloc(u->mx_size,sizeof(__tp_vector));
