                 ...
                 double red, double green, double blue)
{
    ...
    if(colorTable[cindex].name)free(colorTable[cindex].name);
    ...
    if (fontIndex == -1) {
        newFont =
          CreateFont(-size, 0, 0, 0,
                     ...
                     (style & Italic) != 0,
                     0, 0, 0, 0, 0, NONANTIALIASED_QUALITY, FF_DONTCARE, fontName);
        if (newFont != NULL) {
            ...
            if (PrefixMatch(fontName, faceName)
                ...
                pointSize = fontTable[fontIndex].size;
                textStyle = style;
            } else {
...
static void SetLineBB(RECT *rp, double x, double y, double dx, double dy)
{
    ...
    x1 = ScaleX(x + dx);
    y1 = ScaleY(y + dy);
    rp->top = Max(0,Min(y0, y1)-penSize-penSize);
    rp->bottom = Max(y0, y1) + penSize+penSize;
    rp->left = Max(0,Min(x0, x1)-penSize-penSize);
    rp->right = Max(x0, x1) + penSize+penSize;
}
                     ...
                     double rx, double ry, double start, double sweep)
{
    ...
    if (sweep >= 360) {
        SetRect(rp, Max(0,xmin-penSize), Max(0,ymin-penSize), xmax+penSize, ymax+penSize);
        return;
    }
    if (start < 0) {
        start = 360 - fmod(-start, 360);
    ...
    SetRect(rp, Max(0,xl-penSize), Max(0,yt-penSize), xr+penSize, yb+penSize);
}
...
static void SetTextBB(RECT *rp, double x, double y, string text)
{
    ...
    ix = ScaleX(x);
    iy = ScaleY(y);
    SetRect(rp, Max(ix-1,0), Max( iy - textSize.cy + fontTable[currentFont].descent - 1, 0),
            ix + textSize.cx + 1, iy + fontTable[currentFont].descent + 1);
}
...
static void AddPolygonPoint(int x, int y)
{
    ...
    polygonBounds.left = Max(Min(polygonBounds.left, x-penSize),0);
    polygonBounds.right = Max(polygonBounds.right, x+penSize);
    polygonBounds.top = Max(Min(polygonBounds.top, y-penSize),0);
    polygonBounds.bottom = Max(polygonBounds.bottom, y+penSize);
